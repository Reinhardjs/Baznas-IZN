...
public class KDZActionModeCallback implements ActionMode.Callback {

    private Context context;
    private AdapterDampakKajianZakat adapter;
    private ActionMode actionMode;

    public KDZActionModeCallback(Context context, AdapterDampakKajianZakat adapter) {
    ...
    public static ActionMode startActionMode(Activity activity, AdapterDampakKajianZakat adapter){
        KDZActionModeCallback callback = new KDZActionModeCallback(activity, adapter);
            ...
            case R.id.action_edit:
                ...
                if (adapter.getSelectedItemCount() > 1){
                    ...
                    for (int position : adapter.getSelectedItems()) {
                        KajianDampakZakatPojo izn = adapter.getArrayList().get(position);
                        String fk_id = izn.getFk_id();

                        List<KDZ> data = KDZManager.loadByID(context, fk_id);
                        ...
                        try {
                            ...
                            intent.putExtra("data", SetGet.getResponseFromKDZ(data.get(0)));
                ...
                new AlertDialog.Builder(context)
                        ...
                        .setPositiveButton("Ya", new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                for (int position : adapter.getSelectedItems()){
                                    KajianDampakZakatPojo kdz = adapter.getArrayList().get(position);
                                ...
                                for(Iterator<KajianDampakZakatPojo> it = adapter.getArrayList().iterator(); it.hasNext();) {
                                    KajianDampakZakatPojo s = it.next();
                                    if(s == null) {
                                        it.remove();
                                    }
                                }
